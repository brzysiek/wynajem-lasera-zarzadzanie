...
export function SzybkiSmsPanel({ initiallyConfigured }: { initiallyConfigured: boolean }) {
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    const { ok, data } = await postJson(`${BASE_PATH}/api/integrations/szybkisms`, { token });
  ...
  async function handleRemove() {
    ...
    const { data } = await postJson(`${BASE_PATH}/api/integrations/szybkisms`);
  ...
  async function handleTest() {
    ...
    const { data } = await postJson(`${BASE_PATH}/api/integrations/szybkisms/test`, {});
  ...
  return (
    <section className="mb-6 rounded-lg border border-gray-200 bg-white p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">SzybkiSMS</h2>
          <p className="text-sm text-gray-500">API Access Token bramki SMS (wysyłka przypomnień o wynajmach).</p>
        </div>
      ...
      <form onSubmit={handleSave} className="mb-4 flex flex-col gap-2 sm:flex-row">
        <input
          type="password"
          placeholder={configured ? "•••••••••••• (ustawiony — wpisz nowy, żeby zmienić)" : "API Access Token"}
          value={token}
