...
// `paidAt` (RentalFinance) — Fakturownia nie wie, czy faktura jest zapłacona
// (brak płatnego połączenia z bankiem), więc to ustalamy sami (dashboard
// /finanse/faktury, wgrywanie wyciągu bankowego).
  ...
  try {
    ...
    const paidRows = ids.length
      ? await prisma.rentalFinance.findMany({
          where: { fakturowniaInvoiceId: { in: ids } },
