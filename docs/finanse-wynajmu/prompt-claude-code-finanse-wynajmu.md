...
model RentalFinance {
  ...
  capUsedHS  Boolean?
  capFeeNet  Decimal?   // snapshot kwoty z PricingSetting w momencie zaznaczenia (patrz 3.4)
...
### 3.4 Nakładka HS (LightSheer, wariant `double`)
...
- Checkbox `capUsedHS` — domyślnie `null`/nieustawiony, zaznaczany przez kierowcę (dziś nie wiadomo z
  góry, czy nakładka zostanie zużyta).
- Gdy zaznaczony: `capFeeNet` = snapshot wartości `cap_fee_hs_net` z `PricingSetting` **w momencie
  zaznaczenia** (nie licz na żywo z ustawień przy każdym odczycie — cena mogła się zmienić między
  wynajmami, snapshot chroni historyczne dane finansowe przed retroaktywną zmianą).
...
3. Zaznaczenie „Nakładka HS zużyta" → **suma na górze przelicza się natychmiast** (client-side, dla
   odczucia responsywności), niezależnie od pełnego zapisu formularza.
